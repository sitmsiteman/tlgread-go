...
func getTitlesFromIDT(path string) map[string]string {
        ...
        for i := 0; i < len(data)-3; i++ {
                ...
                if data[i] == 0x10 && data[i+1] == 0x01 {
                        length := int(data[i+2])
                        if length == 0 || length > 200 { continue }
                        if i+3+length > len(data) { continue }
...
func main() {
        ...
        if *fPath == "" { log.Fatal("Usage: ./tlgviewer -f tlg[0000-9999].txt [-list] or [-w 1]") }
        ...
        if *list {
                ...
                t := titles[*wID]
                if t == "" { t = titles[normalizeID(*wID)] }
                if t == "" { t = "(Unknown Title)" }
                fmt.Printf("Author: %s\nWork:   %s (ID: %s)\n", author, t, *wID)
                fmt.Println("----------------------------------------")
        }
