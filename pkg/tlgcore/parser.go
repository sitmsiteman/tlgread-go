...
func (p *Parser) parseIDByte() bool {
        if p.Pos >= len(p.Buffer) { return true }
        b := p.Buffer[p.Pos]; p.Pos++
        ...
        case 0xE: // Escape
                if p.Pos >= len(p.Buffer) { return true }
                next := p.Buffer[p.Pos] & 0x7F; p.Pos++
                switch next {
                case 0: level = "a"
                case 1: level = "b"
                case 2: level = "c"
        ...
        case 0xF: // Special
                if right == 0xE { return true } // End Block
                if right == 0x0 { return true } // End File
                return false
        }
        ...
        switch right {
        case 0x0: st.Binary++
        case 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7: st.Binary = int(right); st.ASCII = ""
        case 0x8: st.Binary = p.readBin(1); st.ASCII = ""
        case 0x9: st.Binary = p.readBin(1); st.ASCII = string(p.readChar())
        case 0xA: st.Binary = p.readBin(1); st.ASCII = p.readStr()
        case 0xB: st.Binary = p.readBin(2); st.ASCII = ""
        case 0xC: st.Binary = p.readBin(2); st.ASCII = string(p.readChar())
        case 0xD: st.Binary = p.readBin(2); st.ASCII = p.readStr()
        case 0xE: st.ASCII = string(p.readChar())
        case 0xF: st.Binary = 0; st.ASCII = p.readStr() // ASCII only
        }
...
func (p *Parser) resetLevels(lvl string) {
        ...
        for l, r := range levelRank {
                if r > rank {
                        if resetToNull {
                                p.Levels[l].Binary = 0; p.Levels[l].ASCII = ""; p.Levels[l].Active = false
                        } else {
                                p.Levels[l].Binary = 1; p.Levels[l].ASCII = ""; p.Levels[l].Active = true
                        }
                }
        }
}
...
func normalizeID(id string) string {
        ...
        if err == nil { return strconv.Itoa(i) }
        return id
}
...
func (p *Parser) readText(lim int) string {
        s := p.Pos
        for p.Pos < lim {
                if p.Buffer[p.Pos]&0x80 != 0 { break }
                p.Pos++
        }
...
func (p *Parser) readBin(n int) int {
        v := 0
        for i := 0; i < n; i++ {
                if p.Pos >= len(p.Buffer) { break }
                v = (v << 7) | int(p.Buffer[p.Pos]&0x7F); p.Pos++
        }
        return v
}
func (p *Parser) readChar() rune {
        if p.Pos < len(p.Buffer) { b := p.Buffer[p.Pos]&0x7F; p.Pos++; return rune(b) }
        return ' '
}
func (p *Parser) readStr() string {
        ...
        for p.Pos < len(p.Buffer) {
                b := p.Buffer[p.Pos]
                if b == 0xFF { p.Pos++; break }
                sb.WriteByte(b & 0x7F); p.Pos++
        }
        return sb.String()
}
...
func (p *Parser) formatCitation() string {
        ...
        if len(pts) == 0 { return p.Levels["z"].ASCII }
